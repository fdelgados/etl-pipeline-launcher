...
class SimilarityCalculatorImpl(SimilarityCalculator):
    ...
    def _compute_similarities(self, report: Report, minhashes, pages: pd.DataFrame):
...
def _minhash(tokens: Set[str], num_of_permutations: int = 128) -> LeanMinHash:
    min_hash = MinHash(num_perm=num_of_permutations, hashfunc=xxhash.xxh64_intdigest)
...
def _compute_minhashes(pages: pd.DataFrame, k_shingle_size: KShingleSize) -> List[LeanMinHash]:
    minhashes = []

    for _, row in pages.iterrows():
        minhashes.append(_minhash(_shingle(_tokenize(row['content']), k_shingle_size)))
